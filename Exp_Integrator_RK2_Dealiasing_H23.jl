...
function Exp_Integrator_RK2_Dealiasing_H23(C2, C3, K, a, u0, h, tfin)
    ...
    for n in 1:nsteps
        ...
        u_pos_zeromean = copy(u_pos)
        u_pos_zeromean[1] = 0.0
        xsamp = [real.(u_pos_zeromean[2:end]); imag.(u_pos_zeromean[2:end])]
        H2[n] = C2 * compute_h2(xsamp, K)        # factor of 2 already inside
        H3[n] = -C3 * compute_h3(xsamp, K)       # factor of 6 already inside
        H[n]  = H2[n] + H3[n]
    end 
